#!/usr/bin/env python3
"""
scripts/retranslate3/validate_and_merge3.py
Validates and merges Round 3 translation chunks for Megami Ibunroku Persona.
"""
import json
import sys
...
def validate_chunk(filename, expected_count):
    ...
    if not filepath.exists():
        return False, 0, 0, 0, 0, "MISSING"

    try:
        with open(filepath, "r", encoding="utf-8") as f:
            data = json.load(f)
    except Exception as e:
        return False, 0, 0, 0, 0, f"JSON_ERROR: {e}"

    entries = data.get("entries", [])
    if len(entries) != expected_count:
        return False, len(entries), 0, 0, 0, f"COUNT_MISMATCH ({len(entries)} != {expected_count})"
...
def check_all():
    ...
    print(f"Round 3 Chunks Progress: {done_all}/{total_all} ({pct:.2f}%)\n")

def merge_all():
    print("\n[*] Merging Round 3 chunks into master files...")
